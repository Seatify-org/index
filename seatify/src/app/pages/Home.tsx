...
export default function Home() {
  ...
  const cinemasWithSessions = useMemo(() => {
    return cinemas
      ...
      .map((cinema) => {
        ...
        return { cinema, sessions: cinemaSessions, movieGroups };
      })
      .filter((item) => item.sessions.length > 0)
      .sort((a, b) => (a.cinema.distance || 0) - (b.cinema.distance || 0));
  }, [selectedCity, sessionsForSelectedDate, cinemas]);
  ...
  const hotSessions = useMemo(() => {
    ...
    const result: Array<{
      ...
      movie: ExtendedMovie | undefined;
      cinema: ExtendedCinema | undefined;
      badges: string[];
      urgencyScore: number;
    ...
    for (const s of sessions) {
      if (s.date !== todayStr || s.cinema_city !== selectedCity) continue;
      ...
      const badges: string[] = [];
      if (seatsPercent <= 10) badges.push("last-tickets");
      else if (isAlmostSoldOut) badges.push("almost-sold-out");
      if (isStartingSoon) badges.push("starting-soon");

      let urgencyScore = 500;
      if (badges.includes("last-tickets")) urgencyScore -= 300;
      if (diffMin <= 60) urgencyScore -= 200;
              ...
              <div className="absolute bottom-4 right-4 flex gap-2">
                {[0, 1, 2].map((index) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full transition-all ${currentHeroIndex === index ? "w-6 liquid-gradient" : "glass"}`}
                  />
                ))}
              </div>
            </motion.div>
                ...
                <div key={cinema.id} className="glass-strong rounded-2xl p-4 flex flex-col h-full">
                  ...
                  <div className="flex-1 space-y-2 mb-3">
                    {Object.entries(movieGroups).slice(0, 3).map(([movieId, movieSessions]) => {
                      ...
                      return (
                        <div key={movieId} className="glass rounded-lg p-2">
                          <div className="flex items-center gap-2 mb-1.5">
                            <div className="w-10 h-14 rounded overflow-hidden flex-shrink-0 cursor-pointer hover:scale-105 transition-transform" onClick={() => (window.location.href = `/movie/${movie.id}`)}>
                          ...
                          <div className="flex gap-1.5 flex-wrap">
                            {movieSessions.slice(0, 3).map((session) => (
                              <button key={session.id} onClick={() => (window.location.href = `/movie/${movie.id}/seats?session=${session.id}`)} className="px-2 py-1 glass glass-hover rounded text-xs font-semibold hover:liquid-gradient transition-all">{session.time}</button>
                            ))}
                            {movieSessions.length > 3 && <span className="px-2 py-1 text-xs text-gray-400">+{movieSessions.length - 3}</span>}
                          </div>
                        </div>
                      );
                    })}
                      ...
                      <div className="glass rounded-xl p-4 h-full">
                        ...
                        <div className="space-y-2 mb-3">
                          {Object.keys(movieGroups).slice(0, 2).map((movieId) => {
                            ...
                            if (!movie) return null;
                            return (
                              <div key={movieId} className="text-xs">
                                <div className="font-semibold truncate cursor-pointer hover:text-purple-400 transition-colors" onClick={() => (window.location.href = `/movie/${movie.id}`)}>{movie.title}</div>
                              </div>
                            );
                          })}
            ...
            <div className="overflow-x-auto scrollbar-hide -mx-3 px-3 pb-2">
              <div className="flex gap-3" style={{ width: "max-content" }}>
                {hotSessions.map(({ session, movie, cinema, badges, minutesUntilStart, seatsPercent }) => {
                  ...
                  const borderGlow = isLastTickets ? "border-red-500/40 shadow-[0_0_24px_rgba(239,68,68,0.18)]" : isAlmostSoldOut ? "border-orange-500/40 shadow-[0_0_24px_rgba(249,115,22,0.18)]" : "border-amber-500/25";

                  const timeLabel = isOngoing ? "Идёт сеанс" : minutesUntilStart <= 60 ? `Через ${minutesUntilStart} мин` : `${Math.floor(minutesUntilStart / 60)}ч ${minutesUntilStart % 60}мин`;
