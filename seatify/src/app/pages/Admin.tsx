...
import { movies as srcMovies, cinemas as srcCinemas, sessions as srcSessions } from "../data/movies";
import type { Movie, Cinema, Session } from "../data/movies";
...
// Derive unique halls per cinema from existing sessions
const deriveHalls = (sessions: Session[]) => {
...
type MovieForm = Partial<Movie> & { genreStr?: string; castStr?: string };
type CinemaForm = Partial<Cinema> & { facilitiesStr?: string; tagsStr?: string };

// ─── root ─────────────────────────────────────────────────────────────────────

export default function Admin() {
  const [tab, setTab] = useState<Tab>("movies");
  const [moviesList, setMoviesList] = useState<Movie[]>(srcMovies);
  const [cinemasList, setCinemasList] = useState<Cinema[]>(srcCinemas);
  const [sessionsList, setSessionsList] = useState<Session[]>(srcSessions);

  const hallsMap = useMemo(() => deriveHalls(sessionsList), [sessionsList]);
...
function MoviesTab({
  ...
  const [panelMovie, setPanelMovie] = useState<Movie | "new" | null>(null);
...
function MoviePanel({
  ...
  const set = <K extends keyof MovieForm>(key: K, value: MovieForm[K]) =>
...
function BulkSessionPanel({
  ...
  // Reset hall when cinema changes
  useEffect(() => {
  ...
  const generate = () => {
    ...
    const hallIdFinal = isCustomHall ? `h_${genId()}` : hallId;
...
function AddSessionPanel({
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      movieId,
      cinemaId,
      hallId: isCustom ? `h_${genId()}` : hallId,
      hallName: finalHallName,
      date,
      time,
      price: snapPrice(parseInt(price) || 499),
...
function CinemasTab({
  ...
  const [search, setSearch] = useState("");
  const [panelCinema, setPanelCinema] = useState<Cinema | "new" | null>(null);
...
function CinemaPanel({
  ...
  const set = <K extends keyof CinemaForm>(key: K, value: CinemaForm[K]) =>
    setForm((f) => ({ ...f, [key]: value }));
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      infrastructureTags: (form.tagsStr ?? "").split(",").map((s) => s.trim()).filter(Boolean),
    });
  };
